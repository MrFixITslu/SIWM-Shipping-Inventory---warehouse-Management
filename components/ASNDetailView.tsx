...
const ASNDetailView: React.FC<ASNDetailViewProps> = ({ asn, user, onClose, onEnterFees, onApproveFees, onConfirmPayment, onConfirmArrival }) => {
        ...
        switch (asn.feeStatus) {
            ...
            case FeeStatus.PaymentConfirmed:
                if (asn.status === 'Arrived' || asn.status === 'Processing') {
                    return <p className="text-xs text-center text-green-600 dark:text-green-400">Shipment has arrived and is being processed.</p>;
                }
                ...
                return (
                     <div>
                        ...
                        >
                            <ShipmentIcon className="h-5 w-5 mr-2" /> Receive Shipment
                        </button>
                        {!canWarehouseConfirm && <p className={helperTextClasses}>Cleared for arrival. Waiting for Warehouse team to confirm receipt.</p>}
                    </div>
                );
    ...
    return (
        <div className="mt-6 p-5 bg-white dark:bg-secondary-800 rounded-xl shadow-lg border border-primary-200 dark:border-primary-700/50 animate-modal-appear">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                ...
                {/* Column 3: Actions & Documents */}
                <div className="space-y-3">
                    <h4 className="font-semibold text-secondary-700 dark:text-secondary-300">Actions & Documents</h4>
                    <WorkflowActions />
                </div>
            </div>
        </div>
