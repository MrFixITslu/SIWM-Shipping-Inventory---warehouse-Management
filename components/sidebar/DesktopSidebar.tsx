  ...
  const renderDesktopNavLinks = (items: NavItem[]) => (
    items.map((item) => (
         <NavLink
              ...
              className={({ isActive }) =>
                `${linkClass} ${isActive ? activeLinkClass : inactiveLinkClass} ${isOpen ? '' : 'justify-center relative'}`
              }
            >
              <item.icon className={`${commonIconClass} ${isOpen ? 'mr-3' : 'mr-0'}`} />
              ...
              {!isOpen && item.notificationCount && item.notificationCount > 0 && 
                <span className="absolute top-1 right-1 bg-red-500 text-white text-xs font-bold h-4 w-4 flex items-center justify-center rounded-full">{item.notificationCount}</span>
              }
              {isOpen && item.isBeta && <span className="ml-auto text-xs bg-blue-100 dark:bg-blue-700 text-blue-700 dark:text-blue-200 px-1.5 py-0.5 rounded-full">Beta</span>}
            </NavLink>
    ))
  );
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-30 flex-col bg-white dark:bg-secondary-800 shadow-lg transition-all duration-300 ease-in-out hidden lg:flex ${isOpen ? 'w-64' : 'w-20'}`}>
      <div className={`flex items-center h-16 px-4 border-b border-secondary-200 dark:border-secondary-700 ${isOpen ? 'justify-between' : 'justify-center'}`}>
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-md text-secondary-600 dark:text-secondary-400 hover:bg-secondary-100 dark:hover:bg-secondary-700 focus:outline-none">
