...
import { DashboardMetric, WorkflowMetric, ItemBelowReorderPoint, ItemAtRiskOfStockOut, DashboardStockAnalysis, RunRateData } from '@/types';
...
export const dashboardService = {
  ...
  // New methods for enhanced stock analysis
  getStockAnalysis: async (signal?: AbortSignal): Promise<DashboardStockAnalysis> => {
    return api.get('/dashboard/stock-analysis', { signal });
  },

  getItemsBelowReorderPoint: async (signal?: AbortSignal): Promise<ItemBelowReorderPoint[]> => {
    return api.get('/dashboard/items-below-reorder-point', { signal });
  },

  getItemsAtRiskOfStockOut: async (signal?: AbortSignal): Promise<ItemAtRiskOfStockOut[]> => {
    return api.get('/dashboard/items-at-risk', { signal });
  },

  updateRunRate: async (weeklyInstalls: number, signal?: AbortSignal): Promise<RunRateData> => {
    return api.post('/dashboard/update-run-rate', { weeklyInstalls }, { signal });
  },

  getCurrentRunRate: async (signal?: AbortSignal): Promise<RunRateData> => {
    return api.get('/dashboard/run-rate', { signal });
  }
};