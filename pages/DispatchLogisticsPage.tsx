...
const DispatchLogisticsPage: React.FC = () => {
  ...
  const { isModalOpen: isConfirmReceiptOpen, showConfirmation: showConfirmReceipt, handleConfirm: handleConfirmReceiptAction, handleClose: handleCloseReceiptConfirm } = useConfirmationModal();
    ...
    <PageContainer title="Dispatch & Logistics" actions={ (user?.role === 'admin' || user?.role === 'manager') && <button onClick={() => handleOpenShipmentModal()} className="flex items-center bg-green-500 hover:bg-green-600 text-white font-semibold px-4 py-2 rounded-lg shadow-md" > <PlusIcon className="h-5 w-5 mr-2" /> Create Shipment </button> }>
      ...
      { (user && ['Warehouse', 'admin', 'manager'].includes(user.role)) && (
        <div className="mb-8">
            ...
            <div className="mb-4">
                <input type="text" placeholder="Search orders..." value={searchTermOrders} onChange={(e) => setSearchTermOrders(e.target.value)} className={`block w-full md:w-1/3 pl-4 pr-3 py-2 ${TAILWIND_INPUT_CLASSES}`} />
            </div>
      ...
      <div>
        <h2 className="text-xl font-semibold text-secondary-800 dark:text-secondary-200 mb-3">Outbound Shipments Log</h2>
         <div className="mb-4">
            <input type="text" placeholder="Search shipments..." value={searchTermShipments} onChange={(e) => setSearchTermShipments(e.target.value)} className={`block w-full md:w-1/3 pl-4 pr-3 py-2 ${TAILWIND_INPUT_CLASSES}`} />
        </div>
      ...
      <Modal isOpen={isStatusModalOpen} onClose={handleCloseStatusModal} title={`Update Status for Order #${currentOrderForStatus?.id}`}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <select id="status" value={currentOrderForStatus?.status || ''} onChange={e => setCurrentOrderForStatus(prev => prev ? {...prev, status: e.target.value as OrderStatus} : null)} className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}>
              {Object.values(OrderStatus).map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div className="flex justify-end space-x-3 pt-2">
            <button type="button" onClick={handleCloseStatusModal} className="px-4 py-2 text-sm font-medium rounded-md" disabled={isSaving}>Cancel</button>
            <button type="button" onClick={handleSaveStatusUpdate} className="px-4 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-md disabled:opacity-50" disabled={isSaving}>
              {isSaving ? <LoadingSpinner className="w-5 h-5"/> : 'Save Status'}
            </button>
          </div>
        </div>
      </Modal>

      <Modal isOpen={isShipmentModalOpen} onClose={handleCloseShipmentModal} title={currentShipment.id ? 'Edit Shipment' : (currentShipment.orderId ? `Create Dispatch for Order #${currentShipment.orderId}` : 'Create Inter-Warehouse Shipment')} size="xl">
        <form onSubmit={(e) => { e.preventDefault(); handleSaveShipment(); }} className="space-y-4">
          <ErrorMessage message={isShipmentModalOpen ? error : null} />
          <div> <label htmlFor="brokerId" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">Assign Broker</label>
            <select name="brokerId" id="brokerId" value={currentShipment.brokerId || ''} onChange={(e) => setCurrentShipment({...currentShipment, brokerId: Number(e.target.value)})} required className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}>
                <option value="">Select a Broker</option>
                {brokers.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
          </div>
          <div> <label htmlFor="destinationAddressModal" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">Destination Address</label> <textarea name="destinationAddress" id="destinationAddressModal" value={currentShipment.destinationAddress || ''} onChange={(e) => setCurrentShipment({...currentShipment, destinationAddress: e.target.value})} required rows={3} className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}></textarea> </div>
          <div className="grid grid-cols-2 gap-4"> <div> <label htmlFor="carrierModal" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">Carrier</label> <select name="carrier" id="carrierModal" value={currentShipment.carrier || 'FedEx'} onChange={(e) => setCurrentShipment({...currentShipment, carrier: e.target.value as 'FedEx' | 'UPS' | 'DHL' | 'Other'})} className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}> <option>FedEx</option> <option>UPS</option> <option>DHL</option> <option>Other</option> </select> </div> <div> <label htmlFor="trackingNumberModal" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">Tracking Number</label> <input type="text" name="trackingNumber" id="trackingNumberModal" value={currentShipment.trackingNumber || ''} onChange={(e) => setCurrentShipment({...currentShipment, trackingNumber: e.target.value})} required className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} /> </div> </div>
          <div className="grid grid-cols-2 gap-4"> <div> <label htmlFor="dispatchDateModal" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">Dispatch Date</label> <input type="date" name="dispatchDate" id="dispatchDateModal" value={currentShipment.dispatchDate || ''} onChange={(e) => setCurrentShipment({...currentShipment, dispatchDate: e.target.value})} required className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} /> </div> <div> <label htmlFor="estimatedDeliveryDateModal" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">Est. Delivery Date</label> <input type="date" name="estimatedDeliveryDate" id="estimatedDeliveryDateModal" value={currentShipment.estimatedDeliveryDate || ''} onChange={(e) => setCurrentShipment({...currentShipment, estimatedDeliveryDate: e.target.value})} required className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} /> </div> </div>
          
          <div className="flex justify-end space-x-3 pt-2"> <button type="button" onClick={handleCloseShipmentModal} className="px-4 py-2 text-sm font-medium text-secondary-700 dark:text-secondary-300 bg-secondary-100 dark:bg-secondary-600 hover:bg-secondary-200 rounded-md" disabled={isSaving}>Cancel</button> <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-md disabled:opacity-50" disabled={isSaving}> {isSaving ? <LoadingSpinner className="w-5 h-5"/> : (currentShipment.id ? 'Save Changes' : 'Create Shipment')} </button> </div>
        </form>
      </Modal>

      <ConfirmationModal isOpen={isConfirmDeleteOpen} onClose={handleCloseDeleteConfirm} onConfirm={handleConfirmDelete} title="Delete Dispatch" message="Are you sure you want to delete this dispatch record?" />
      <ConfirmationModal isOpen={isConfirmReceiptOpen} onClose={handleCloseReceiptConfirm} onConfirm={handleConfirmReceiptAction} title="Confirm Order Receipt" message="Are you sure you want to confirm receipt of all items for this order? This will mark the order as Completed." confirmButtonText="Confirm Receipt" />
      <Modal isOpen={isRouteOptimizationModalOpen} onClose={() => setIsRouteOptimizationModalOpen(false)} title="AI Action Confirmation"> <p>{confirmationModalMessage}</p> <div className="flex justify-end pt-4"> <button onClick={() => setIsRouteOptimizationModalOpen(false)} className="px-4 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-md">OK</button> </div> </Modal>
