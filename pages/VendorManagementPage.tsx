...
const VendorManagementPage: React.FC = () => {
  ...
  const handleSaveVendor = async () => {
    ...
    try {
      ...
      // No need to fetchVendors(), context handles the update via SSE
      handleCloseModal();
    } catch (err: any) {
    ...
    >
      ...
      <div className="mb-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search vendors (name, contact, email, products)..."
            value={searchTerm}
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={currentVendor.id ? 'Edit Vendor' : 'Add New Vendor'} size="lg">
        <form onSubmit={(e) => { e.preventDefault(); handleSaveVendor(); }} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" name="name" id="vendorName" value={currentVendor.name || ''} onChange={(e) => setCurrentVendor({...currentVendor, name: e.target.value})} required className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="email" name="email" id="email" value={currentVendor.email || ''} onChange={(e) => setCurrentVendor({...currentVendor, email: e.target.value})} required className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="phone" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">Phone</label>
              <input type="tel" name="phone" id="phone" value={currentVendor.phone || ''} onChange={(e) => setCurrentVendor({...currentVendor, phone: e.target.value})} className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} />
            </div>
             <div>
              ...
              <input type="date" name="lastCommunicationDate" id="lastCommunicationDate" value={currentVendor.lastCommunicationDate || ''} onChange={(e) => setCurrentVendor({...currentVendor, lastCommunicationDate: e.target.value})} className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} />
            </div>
          </div>
          <div>
            ...
            <input type="number" name="performanceScore" id="performanceScore" value={currentVendor.performanceScore ?? ''} min="0" max="100" onChange={(e) => setCurrentVendor({...currentVendor, performanceScore: parseInt(e.target.value)})} className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} />
          </div>
          <div>
            <AutocompleteInput
