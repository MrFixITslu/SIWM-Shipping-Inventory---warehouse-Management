...
const WarehouseOrdersPage: React.FC = () => {
  ...
  const { isModalOpen: isConfirmPickupOpen, showConfirmation: showConfirmPickup, handleConfirm: handleConfirmPickupAction, handleClose: handleClosePickupConfirm } = useConfirmationModal();
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      const validItems = currentOrder.items.filter(itemFS => itemFS.itemId);
  ...
  return (
    ...
    >
      ...
      <ConfirmationModal isOpen={isConfirmDeleteOpen} onClose={handleCloseDeleteConfirm} onConfirm={handleConfirmDelete} title="Delete Warehouse Order" message="Are you sure you want to delete this order? This action cannot be undone." />
      <ConfirmationModal isOpen={isConfirmPickupOpen} onClose={handleClosePickupConfirm} onConfirm={handleConfirmPickupAction} title="Confirm Order Pickup" message="Are you sure you have picked up all items for this order? This will update the status to 'Picked Up'." confirmButtonText="Confirm Pickup" />
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSaveOrder(); }} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="department" id="department" value={currentOrder.department || ''} onChange={(e) => setCurrentOrder({...currentOrder, department: e.target.value})} required className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`} />
            </div>
            <div>
              ...
              <select name="technicianId" id="technicianId" value={currentOrder.technicianId || ''} onChange={(e) => setCurrentOrder({...currentOrder, technicianId: Number(e.target.value)})} className={`mt-1 block w-full ${TAILWIND_INPUT_CLASSES}`}>
